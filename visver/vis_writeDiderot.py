...
def inShape(f,appC):
    ...
    for exp in exps:
        ...
        if (field.get_isField(exp)):
            ...
            #foo= fieldName(i)+" = "+krnstr+u'⊛'+"  image(\"symb/sphere.nrrd\");\n"
            f.write(foo.encode('utf8'))
        else: #tensor type
...
# change here for MIP: must be a real
...
def readDiderot(p_out,app,pos,template):
    ...
    for line in ftemplate:
        ...
        if d0:
            ...
            continue
        g0 = re.search(foo_fieldOut, line)
        if g0:
...
def runDiderot(p_out, app, pos, output, runtimepath, isNrrd, t_templatesize, nrrdname ):
    print "**********  runDidero***********"
    shape = app.oty.shape
    # print "shape",shape
    product = 1
    ...
    #print "shape: ", shape
    size = str(300)
    PARAMS = " -camOrtho true -camEye 8 0 0 -camFOV 15 -rayStep 0.01 -iresU "+size+" -iresV  "+size
    if(isNrrd):
...
def writeDiderot(p_out, app, pos, output, runtimepath, isNrrd,t_templatesize,t_templatefile):
    ...
    if(not(os.path.exists(p_out))):
        ...
        os.system("rm "+txfile)
        # run executable
        runDiderot(p_out, app, pos, output, runtimepath, isNrrd, t_templatesize, nrrdname)
