...
def core_ops(exps, dim, tshape1, tshape2, opr_inner, opr_outer, title, testing_frame, cnt):
    ...
    (app, coeffs) = create_apply2(exps, tshape1, tshape2, opr_inner, opr_outer,  testing_frame)
    counter.inc_cumulative(cnt)
    core2(app, coeffs, dimF, title, testing_frame, cnt)
    print "done core-ops"
    return
...
def pick_ty_to_exp3(exps, dim, ty3s, tshape1, tshape2_shape, opr_inner, opr_outer, title, testing_frame, cnt):
    ...
    for  i in exps:
        print "i:",i.name
    print "tshape1: ", tshape1.name, " k:",tshape1.k
    ...
    def instance(exp3, dim):
        ...
        if(exp3 == None):
            ...
            tshape2 = check2_tshape(opr_outer, tshape1, exp3, tshape2_shape, dim)
            print "tshape2:", tshape2
            if(not(tshape2 == None)):
                print "inside if"
                # both operations are binary so there are three expressions
                core_ops(exp_t, dim, tshape1, tshape2, opr_inner, opr_outer, title, testing_frame, cnt)
                return
            else:
                print "inside else"
    if(dim==0):
        ...
        if(exp2 == None):
            ...
            if(not (tshape1==None)):
                ...
                # check to see if it is expression (tshape1) is a  differentiable field (if needed)
                    tshape2 = check1_tshape(opr_outer,  tshape1, tshape2_shape, dim)
                    if(not(tshape2 == None)):
...
# main function
def pick_get_ty2s(ty1s, rtn2s, opr_inner, opr_outerO, title, testing_frame, cnt):
    # print " --------------second  time---------------"
...
def iter_inner_unary(ty1s, shape1, opr_inner, opr_outer, title, testing_frame, cnt):
    def iter_unary_unary(opr_outer):
        tshape1_shape = op1_to_shape(opr_inner, shape1)
        print "------------------------------------------------------"
        print "opr_outer.arity == 1 and opr_inner.arity == 1"
        print "inner", opr_inner.name, "outer", opr_outer.name
    ...
    def iter_binary_unary(opr_outer):
        ...
        if(not (tshape1_shape == None)):
            print "op2_to_shape","opr_outer: ",opr_outer.name," tshape1_shape: ", tshape1_shape.name
            ...
            if(not(tshape1_shape.id ==  ty_noneT.id)):
                ...
                print "tshape1_shape ==  ty_noneT", opr_outer.name, tshape1_shape.name," tshape1_shape", tshape1_shape.shape,"ty_noneT", ty_noneT.name,"ty_noneT.shape", ty_noneT.shape
                print "AB"
                ...
                for exp1 in ty1s:
                    ...
                    if(not (exp1==None)):
                        ...
                        if(not (tshape1 == None)):
                            ...
                            print "inner;", opr_inner.name, "outer:",opr_outer.name,"tshape1_shape: ",tshape1_shape.name,"tshape1_shape-shape: ",tshape1_shape.shape
    ...
    def sort(opr_outer):
        if(opr_inner.arity == 1 and opr_outer.arity == 1): # arity is 1
            iter_unary_unary(opr_outer)
        elif(opr_outer.arity == 2 and opr_inner.arity == 1):
            iter_binary_unary(opr_outer)
    ...
    if(opr_outer==None):
        ...
        for  t_outer in range(n):
            ...
            sort(opr_outer)
            endouter = time.time()
            ttouter ="shape-> inner->outer"+opr_inner.name+"_"+opr_outer.name+"  time: "+str(endouter - startouter)
            writeall(ttouter)
            print (ttouter)
    else:
        sort(opr_outer)
...
# main function
def pick_get_ty1s(ty1s, shape1, opr_inner, opr_outer, testing_frame, cnt):
    ...
    if(opr_inner.arity == 1): # arity is 1
        ...
        iter_inner_unary(ty1s, shape1, opr_inner, opr_outer, title, testing_frame, cnt)
    else:
...
def go_backwards(opr_outer, opr_inner, testing_frame, cnt):
    k_init = 2
    print "opr_outer: ",opr_outer.name, " opr_inner: ",opr_inner.name
    rst2  =  backward_op1(opr_outer,  k_init)
    ...
    for  (tshape1, tshape2) in rst2:
        ...
        for exp  in ty1s:
            ...
            if(not(shape2==None)):
                print "rst2:##########################",opr_outer.name,"(",tshape1.name,") ->",tshape2.name
                print "rst2:","(",exp1.name,opr_inner.name,shape2.name,") ->",tshape1.name
                print "rst2:",opr_outer.name,"(",exp1.name,opr_inner.name,shape2.name,") ->",tshape2.name
                
                #tt ="\t\tex.(exp1:"+exp1.name+", shape2:"+shape2.name+")-> "+tshape2.name
                #print(tt)
                #print "dim: ", dim, "tshape1:",tshape1.name, "tshape2:",tshape2.name
                #print "outer: ", opr_outer.name,"tshape1:",tshape1.name,"-->tshape2:",tshape2.name
        ...
        if(is_nrrd(shape1)):
            ...
            for t_inner in range(n):#n):
                ...
                for t_outer in range(n):
                    ...
                    pick_get_ty1s(ty1s, shape1, opr_inner, opr_outer, testing_frame, cnt)
                    endxx = time.time()
                    ttxx = "shape-> inner->..."+opr_inner.name+" time: "+str(endxx - startxx)
                    writeall(ttxx)
                    print (ttxx)
                    write_results("done specific inner "+opr_inner.name+" for a shape "+shape1.name, testing_frame, cnt)
                    writeCumulative(cnt)
                    endx = time.time()
...
# inner-> outer -> shape
def pick_inner(opr_inner, opr_outer, testing_frame, cnt):
    #writeTitle_outer(opr_inner, opr_outer)
    # iterate over shapes for first terms
    shapes = tvs+tms#+tt2+tt3+tt4
    ...
    for shape1 in shapes:
        ...
        pick_get_ty1s(ty1s, shape1, opr_inner, opr_outer, testing_frame, cnt)
    endall = time.time()
    ttall = "inner-> outer -> shape: "+ str(endall - startall)
    writeall(ttall)
    print (ttall)
    #print "forwards"
...
def pick_backwards(opr_inner, opr_outer, testing_frame, cnt):
    #writeTitle_outer(opr_inner, opr_outer)
    if((opr_inner.arity == 2) and opr_outer.fieldop):
        print "mark-a"
        go_backwards(opr_outer, opr_inner, testing_frame, cnt)

    elif(opr_inner.arity == 2) and (opr_outer.arity == 1):
        # assume outer is op1
        # assumes inner is op2
        print "mark-b"
        go_backwards(opr_outer, opr_inner, testing_frame, cnt)
    else:
        print pick_inner(opr_inner, opr_outer, testing_frame, cnt)
    return
